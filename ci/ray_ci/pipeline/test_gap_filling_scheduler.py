...
def test_get_commits_between_passing_and_failing(
    mock_init_buildkite, mock_get_builds, mock_check_output
):
    ...
    scheduler = GapFillingScheduler("org", "pipeline")
    assert scheduler._get_gap_revisions() == ["222", "333"]


@mock.patch("ray_ci.pipeline.gap_filling_scheduler.GapFillingScheduler._init_buildkite")
@mock.patch(
    "ray_ci.pipeline.gap_filling_scheduler.GapFillingScheduler._get_gap_revisions"
)
@mock.patch("ray_ci.pipeline.gap_filling_scheduler.GapFillingScheduler._create_build")
def test_run(mock_init_buildkite, mock_get_gap_revisions, mock_create_build):
    mock_get_gap_revisions.return_value = ["222", "333"]
    scheduler = GapFillingScheduler("org", "pipeline")
